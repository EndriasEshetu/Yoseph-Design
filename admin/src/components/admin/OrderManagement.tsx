                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem
                          onClick={() =>
                            handleStatusUpdate(order.id, "processed")
                          }
                          className="gap-2"
                        >
                        ...
                        <DropdownMenuItem
                          ...
                          className="gap-2"
                        >
                          <Truck className="w-4 h-4 text-purple-500" /> Mark as
                          Shipped
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() =>
                          ...
                          className="gap-2"
                        >
                          <Package className="w-4 h-4 text-green-500" /> Mark as
                          Delivered
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
      ...
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Order ID: {selectedOrder?.id} • Placed on{" "}
              {selectedOrder && format(new Date(selectedOrder.date), "PPP p")}
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="space-y-3">
                  {selectedOrder.items.map((item, idx) => (
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span className="w-4 h-4 flex items-center justify-center bg-neutral-100 rounded text-[10px] font-bold">
                          {item.quantity}
                        </span>
                        {item.name}
                      </span>
                  ...
                  <div className="pt-3 border-t border-neutral-100 flex justify-between items-center font-bold">
                    ...
                    <span className="text-lg">
                      Br {selectedOrder.total.toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
