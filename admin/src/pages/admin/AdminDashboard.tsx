    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={`Br ${(totalRevenue / (totalOrders || 1)).toFixed(0)}`}
          trend="-2.4%"
          trendUp={false}
...
const StatCard = ({ title, value, trend, trendUp, icon }: any) => (
  <div className="bg-white border border-neutral-200 p-6 rounded-xl shadow-sm">
    <div className="flex items-start justify-between mb-4">
      ...
      <div
        className={`flex items-center gap-1 text-[10px] font-bold ${trendUp ? "text-emerald-500" : "text-rose-500"}`}
      >
        {trendUp ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}
        {trend}
      </div>
    </div>
    <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400 mb-1">
      {title}
    </h4>
