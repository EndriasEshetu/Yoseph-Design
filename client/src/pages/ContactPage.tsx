...
export const ContactPage = () => {
  ...
  const validateEmailFormat = useCallback((value: string) => {
    ...
    const rules: Record<
      string,
      { min: number; pattern?: RegExp; label: string }
    > = {
    ...
    if (rule) {
      if (local.length < rule.min) {
        setEmailError(
          `${rule.label} usernames must be at least ${rule.min} characters.`,
        );
        return;
      }
      if (rule.pattern && !rule.pattern.test(local)) {
        setEmailError(
          `${rule.label} usernames can only contain letters, numbers, and dots.`,
        );
        return;
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-1">
              <label
                ...
                className="text-[10px] uppercase font-bold text-neutral-600"
              >
                Category
              </label>
